...
function RegisterPageClient({ key }: { key: string }) {
  ...
  React.useEffect(() => {
    if (isAndroid) {
      //  window.location.href = `b3well://url=${url}`;
  ...
  // Desktop
  const universalUrl = `https://${WEB_HOST}/r/${encodeURIComponent(key)}`;
  return (
